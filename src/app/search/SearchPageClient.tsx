...
import { collection, query, getDocs, limit as firestoreLimit, Query } from 'firebase/firestore';
...
interface ThemeAwareLoader {
  isLoading: boolean;
  loadingText: string | null;
  theme: string;
  toggleTheme: () => void;
  onDataLoad: (message: string) => Promise<void>;
}

export default function SearchPageClient() {
  const { isLoading, loadingText, theme, toggleTheme, onDataLoad } = useThemeAwareLoader() as ThemeAwareLoader;
  ...
  const performSearch = async (queryString: string) => {
    ...
    try {
      ...
      const gamesQuery: Query = applyLimit === firestoreLimit
      ...
      const games = gamesSnapshot.docs
        .map((doc) => ({
          ...
          console: doc.data().consoleId as string | undefined,
          players: `${doc.data().playingCount || 0} playing` as string,
          image: (doc.data().coverImageUrl as string) || 'https://via.placeholder.com/300',
        }))
        .filter((game) => game.title && game.title.toLowerCase().includes(lowerQuery));

      // Search Consoles
      const consolesRef = collection(db, 'consoles');
      const consolesQuery: Query = applyLimit === firestoreLimit
      ...
      const consoles = consolesSnapshot.docs
        .map((doc) => ({
          ...
          games: `${doc.data().gamesCount || 0} Games` as string,
          tooltip: `Browse ${doc.data().gamesCount || 0}+ ${doc.data().name} games` as string,
          image: (doc.data().imageUrl as string) || `/images/${doc.data().shortName}-Controller-Flat.png`,
        }))
        .filter((console) => console.name && console.name.toLowerCase().includes(lowerQuery));

      // Search Users
      const usersRef = collection(db, 'users');
      const usersQuery: Query = applyLimit === firestoreLimit
      ...
      const users = usersSnapshot.docs
        .map((doc) => ({
          id: doc.id,
          username: (doc.data().username as string) || (doc.data().email as string).split('@')[0],
          email: doc.data().email as string,
          photoURL: (doc.data().photoURL as string) || 'https://via.placeholder.com/150',
        }))
