  ...
  if (error) {
    return (
      <div className="container text-center py-5">
        <h1 className={theme === 'dark' ? 'text-light' : 'text-dark text-danger'}>{error}</h1>
      </div>
    );
  }

  if (isLoading) {
    return <PacmanLoader message={loadingText || loadingTitle || 'Loading Admin Portal Data'} />;
  }
      ...
      <div className="container py-5">
        ...
        <div className="nav nav-tabs mb-4">
          {['users', 'games', 'consoles', 'analytics'].map((tab) => (
            <button
              key={tab}
              className={`nav-link flex-fill text-center ${activeTab === tab ? 'active' : ''}`}
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.id}>
                        ...
                        <td>
                          <button
                            className={`btn btn-sm ${u.admin ? 'btn-warning' : 'btn-success'} me-2`}
